...
void controlMotor(const int comando) {
  if ((comando > 0) && (posicionActual < 360 - angle)) { // Comprobamos que estamos dentro del rango
    Serial.println("Moving " + String(angle) + " degrees");
    myStepper.step(degree2steps(angle));
    posicionActual = posicionActual + angle;
    Serial.println("Movement completed");
  } else if ((comando < 0) && (posicionActual > 0 + angle)) {  // Comprobamos que estamos dentro del rango
    Serial.println("Moving " + String(angle) + " degrees");
    myStepper.step(degree2steps(-angle));
    posicionActual = posicionActual - angle;
    Serial.println("Movement completed");
